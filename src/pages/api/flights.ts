...
export default function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  const { query } = req;
  const { direction, searchString, flightDate } = query as Query;

  let flights = allFlights[direction];

  console.log({ flightDate, searchString });

  if (!flightDate && !searchString) {
    res.status(200).json({ flights });
    return;
  }

  if (flightDate) {
    const plannedDate = new Date(flightDate);

    flights = flights.filter((flight) => new Date(flight.date).getTime() === plannedDate.getTime());
  }

  if (searchString) {
    const stringToSearchFor = searchString.replace(' ', '').toLowerCase();

    flights = flights.filter((flight) => {
      const flightNumber = flight.flightNumber.replace(' ', '').toLowerCase();
      const airport = flight.airport.replace(' ', '').toLowerCase();

      return flightNumber === stringToSearchFor || airport === stringToSearchFor;
    });
  }
